...
interface EssentialExpensesCardProps {
  essentialExpenses: Entry[];
}

export function EssentialExpensesCard({ essentialExpenses }: EssentialExpensesCardProps) {
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/90 p-5 shadow-lg backdrop-blur-sm">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-zinc-100">Essential Expenses</h3>
        </div>
        <span className="rounded-md bg-zinc-800 px-2 py-0.5 text-xs font-medium text-zinc-400">
          {essentialExpenses.length} {essentialExpenses.length === 1 ? "item" : "items"}
        </span>
      </div>

      {essentialExpenses.length === 0 ? (
        <p className="mt-4 text-sm text-zinc-500 italic">No essential expenses recorded yet.</p>
      ) : (
              ...
              >
                ...
                <span
                  className={`text-sm font-semibold ${
                    isEstimated ? "text-amber-400 italic" : "text-zinc-200"
                  }`}
                >
