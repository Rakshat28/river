...
const appliedV7 = sessionReducer(null, { type: "SET_STATE", payload: stateV7 });
assert(appliedV7 !== null && appliedV7.state_version === 7, "Failed to set initial state to v7");
...
assert(resultAfterV5?.state_version === 7, "State version changed after receiving lower version");
...
assert(resultAfterV8 !== null && resultAfterV8.state_version === 8, "Reducer failed to update to higher state_version 8");
...
assert(resultAfterHydrate !== null && resultAfterHydrate.state_version === 3, "HYDRATE failed to apply over existing state");
